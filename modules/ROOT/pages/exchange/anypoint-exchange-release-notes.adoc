...
* Improve user experience for direct upload by providing relevant information about the files that should be uploaded for each asset type. (EXC-3427)
...
* Provide correct error information to the Maven CLI so that Maven can retry the request with proper credentials instead of failing. (EXC-3520)
...
* Unauthenticated access to portal images is disabled for all new uploaded images. Existing images previous to this change must be re-uploaded for this fix to take effect. (EXC-3394)
