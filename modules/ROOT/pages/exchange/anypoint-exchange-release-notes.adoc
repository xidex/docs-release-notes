...
* Recently viewed assets are displayed in Exchange on the navigation bar's Recent Items list. (EXC-3264)
...
* Users can now publish RAML files from the Anypoint CLI. (EXC-2539)

=== Fixed in this Release

* For better security, the user's authentication token is no longer included in the request header in each request made with API Console. (EXC-3416)
* Enable filtering assets by type on Public portals. Previously all assets were shown, disregarding type filter selection. (EXC-3400)
* Make Terms and Conditions visible in all cases. (EXC-3383)
* Add `jpeg` to the list of supported extensions for portal resources (`bmp`, `gif`, `ico`, `jpe`, `jpeg`, `jpg`, `png`, `tif`, `tiff`, `webp`). (EXC-3351)
* Protect service by setting a max number (50) of pages that can be created in the asset documentation. (EXC-3157)
* Fix validation for publishing API specifications from xAPI so all specifications have an associated file. (EXC-2541)
