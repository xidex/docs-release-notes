...
In Runtime Fabrics that are enabled with Anypoint Security, you can manage blocked IP addresses. For more information, see xref:anypoint-security::manage-blocked-ip-in-rtf.adoc[Manage Blocked IP Addresses in Runtime Fabric]. 
...
* The issue where Edge failed to route requests to the Mule when the client (through the request header) asserts `Transfer-Encoding: chunked` is fixed.
