...
When users of business groups accessed the Secrets Manager user interface, they got a `do not have permission` error when trying to perform any action in Secrets Manager. This happened even when the master organization (or parent of a business group) had the *API Manager Environments* entitlement. 
