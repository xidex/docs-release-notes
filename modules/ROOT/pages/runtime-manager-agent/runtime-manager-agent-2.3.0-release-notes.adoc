...
* API Manager secrets (`clientId`, `clientSecret`, and `Proxy password`) can now be stored, encrypted, in the `wrapper.conf` file when an encryption key is provided at setup time.
