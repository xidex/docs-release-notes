...
* Atomic schedulers modifications: modifying schedulers' settings will be done atomically using a new HTTP endpoint in the Mule Agent API without triggering the other schedulers deployed to the same Mule Runtime. Prior to this update, modifications to schedulers forced the synchronous trigger of the schedulers of all applications running in the same runtime.
