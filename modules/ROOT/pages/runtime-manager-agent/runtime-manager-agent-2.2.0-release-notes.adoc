...
When you upgrade the agent, two new endpoints are created for communicating with Anypoint Runtime Manager. Before upgrading, ensure there are no networking constraints for accessing the FQDNs on port 443.
...
* `+https://data-authenticator.anypoint.mulesoft.com+` a|
