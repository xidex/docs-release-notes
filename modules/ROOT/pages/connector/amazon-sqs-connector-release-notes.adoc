...
* Connector continues retrieving messages when flow is stopped. +
In previous versions, the connector continued retrieving messages after the Mule application stopped. In teh current version, after the Mule application stops, the connector stops retrieving messages.
* Multiple connections established. +
Previously, the connector would make a high number of connections, even if the polling was set to a high value. This is now fixed.
* Connector stops working after receiving an error 500 from Amazon. +
The connector stopped receiving messages when a status code 500 was received from Amazon. This was fixed, and the connector now attempts a reconnection in cases like this.
