...
* The conector would occasionally incorrectly assume that the input payload was empty. This was fixed and now works regardless of the type of InputStream that the connector's operations receive. (SE-12156)
...
* *Response Time* is now configurable. This field is located under *Additional Settings* in the connector's configuration.
This timeout is the maximum amount of time in milliseconds that a managed connection attempts to wait for a response to a request. A value of 0 represents no timeout. The default is 30000 milliseconds (30 seconds).
