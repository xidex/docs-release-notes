...
* Corrected datasense metadata (removed "Heading" level shown in datasense but not actually used in data)
* Added support for all standard escape sequences, except those which change character encoding (will not be supported)
* Fixed writing a String value to a timestamp field (by default expects XMLGregorianCalendar, but correctly formatted String also supported)
