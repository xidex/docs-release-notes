...
* Fixed a bug that made the list operation fail when listing a directory found a file which was later moved or deleted before reading it. (MULE-16766)
* Fixed a bug that made the listener abort a poll when failing to list a file. Now the connector either lists all files, or fails entirely. (MULE-15792)
