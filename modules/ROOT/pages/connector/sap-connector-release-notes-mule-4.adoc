...
* Added the option to use default SAP format for date and time when working with BAPIFunctions over sRFC.

=== Fixed in this Release

* A new parameter in SAP Outbound configuration was added in Advanced tab. The parameter name is: "Use SAP date time format" and has false as default value. If true the DATE and TIME fields will be formatted as strings (yyyy-MM-dd / HH:mm:ss); if false their value will be returned. (SE-11815)

* Fixed SAP Connector not accepting special character for incoming BAPI function RFC response. (SE-11651)

* Configuring SNC in SAP connector not working. Added message server configuration parameters. (SE-10985)
