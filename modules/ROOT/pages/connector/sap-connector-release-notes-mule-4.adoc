...
* SAP Connector - Wrong number format returned (SE-12022). Fixed by adding new parameter useSAPFormat which if set to true, will return default values received from SAP.
* SAP Connector - Parsing error when SAP response contains nested tables. (SE-11910)
* Fixed SAP Connector vulnerable to XML External Entity (XXE).
* Fixed SAP Connector Encoding field needs hint description. 
