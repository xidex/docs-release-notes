...
** EE-6206: PGPEncrypter not adding Armor-ASCII output for string payloads 
** SEC-330: TokenStore should remove expired tokens
...
** EE-6223: OAuth Provider: token request for clientCredentials with encoded credentials fails 
** EE-6224: OAuth Provider: token response body is json, but content-type header is application/x-www-form-urlencoded
...
|1.8.0 |3.9.0 or newer
|1.7.4 |3.9.0 or newer
