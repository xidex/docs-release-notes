...
IMPORTANT: The NetSuite version is not the same as the SuiteTalk API version. Your account may be running a different version of NetSuite than the version of the API you are using.
...
* On certain scenarios, calls to the *Get Select Value* operation used to fail with a `NullPointerException`. This was fixed.
