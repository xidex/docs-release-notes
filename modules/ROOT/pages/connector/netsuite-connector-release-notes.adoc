= NetSuite Connector Release Notes for Mule 3
...
NOTE: The NetSuite connector version is not the same as the SuiteTalk API version. Your account may be running a different version of NetSuite than the version of the API you are using.
...
|===
|Software |Version
|Mule Runtime|3.5 and later Mule 3 versions
|Java|7
...
* In certain scenarios, calls to the `Get Select Value` operation failed with a `NullPointerException`.
...
* Boolean and date values were incorrectly mapped when processing an API request.  
