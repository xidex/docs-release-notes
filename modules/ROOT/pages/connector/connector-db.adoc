...
* On stored procedures where the output return variable is set to null, the output should not be processed further. (https://www.mulesoft.org/jira/browse/MULE-16961[MULE-16961])
* db:select operation fails to throw a connectivity error (https://www.mulesoft.org/jira/browse/MULE-16950[MULE-16950])
...
* Connector doesn't ensure data is consumable by DataWeave if it has an encoding different from the one used by the runtime. (https://www.mulesoft.org/jira/browse/MULE-16460[MULE-16460])
* Clob in out parameters are not made consumable with Oracle DB. (https://www.mulesoft.org/jira/browse/MULE-16770[MULE-16770])
