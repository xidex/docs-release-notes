...
* Updated code to use the `poll(java.time.Duration)` method provided in the new library to replace the deprecated `poll(long)` method. The deprecated method did not block beyond the timeout awaiting partition assignment. (KIP-266)
