...
* If the first cluster node in the list is down the connection fails regardless of the status of the 2nd node in the list - Fixed this issue by adding an optional parameter to the advanced configuration tab. Now the users can specify the Load Balancing Policy by using the field with the same name. Using a Round Robin policy fixes this issue. (SE-11191)
