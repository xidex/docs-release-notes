...
* OAuth v2.0 leverages refresh_token scenario for a specific set of operations (SE-11869) :

_CoreAPI_: create, createMultiple, createSingle,update, updateMultiple, updateSingle, upsert, upsertMultiple, merge, retrieve,
delete, deleteMultiple, convertLead, findDuplicatesByIds, findDuplicates, getSessionId, getUserInfo, resetPassword,
changeOwnPassword, getUpdated, getUpdatedRange, getUpdatedObjects, getDeleted, getDeletedRange, setPassword, getServerTimestamp,
nonPaginatedQuery, query, queryAll, querySingle, search, describeSobject, describeGlobal

_MetadataAPI_: deployMetadata, createMetadata, updateMetadata, deleteMetadata, upsertMetadata, renameMetadata, readMetadata,
describeMetadata, listMetadata, retrieveMetadata
...
* Subscribe to a filtered topic not working with v9.7.0 - The Salesforce connector now supports subscribing to a topic using a filter with the syntax `topicName?filterKey=filterValue`. For more information, see the Salesforce https://developer.salesforce.com/docs/atlas.en-us.api_streaming.meta/api_streaming/using_filtered_subscriptions.htm[Filtered Subscriptions] page. (SE-11869)
