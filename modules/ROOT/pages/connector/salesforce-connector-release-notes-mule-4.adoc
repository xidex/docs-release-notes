...
* Metadata issues for sources(ONN-1918) - The keys and output datasense were wrong for the SubscribeTopic, SubscribeChannel, ReplayTopic and ReplayChannel sources. With this version metadata was fixed.
* Duplicate values with objectType case(CONN-451) - There were some issues in the input/output datasense for different objects where some of the fields were duplicated.
...
** Commons BeanUtils Core(`commons-beanutils:commons-beanutils-core:1.8.3`), BeanShell(org.beanshell:bsh-core:2.0b4) and ESAPI(`org.owasp.esapi:esapi:2.1.0.1`) dependencies were removed as they were not used anymore.
