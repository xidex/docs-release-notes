...
* Metadata for Apex Classes would never work if the instance would have more than 200 records. Fixed by using the search service to query the instance for apex classes instead of simple query.
* Null values are set as blank by Dataweave when obtained from Salesforce Connector. Fixed by changing the returning JSONObject.NULL fields to null.
* Changed the api call that is performed to validate the connection to improve the connector performance.
