...
* Salesforce invokes Apex metadata request for each invoke. (SE-10925) - Now the connector does not fetch all the metadata to validate the connection, it only performs a Get Server Timstamp operation which should tell the app if reconnection is necesarry or not.
* Vulnerability reported for Salesforce connector 9.2.2. (SE-9824) - A part of the vulnerabilities reported were fixed by upgrading some of the dependencies the connector uses.
