...
* Subscribe to filtered topic not working with v9.7.0 - This issue was fixed and now the connector supports subscribing to a topic using a filter with the following syntax topicName?filterKey=filterValue. For more details regarding this type of subscription visit https://developer.salesforce.com/docs/atlas.en-us.api_streaming.meta/api_streaming/using_filtered_subscriptions.htm[Filtered Subscriptions] page. (SE-11869)
