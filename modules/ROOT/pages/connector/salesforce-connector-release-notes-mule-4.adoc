...
* Fixed an issue on Create Batch Stream operation where the connector was not able to reconnect after the session became invalid
* Fixed an issue on the Query operations where the execution was stopping if the query was invalid. Now a proper error is raised.
