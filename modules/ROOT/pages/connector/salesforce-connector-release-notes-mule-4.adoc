...
* Mutual TLS support(CONN-1142) - Mutual TLS support was added for all the connection providers

=== Fixed in this Release

* Salesforce Subscribe topic connector is disconnected (SE-11365) - We have upgraded the CometD library(org.cometd.java:cometd-java-client:3.0.10) to a newer version org.cometd.java:cometd-java-client:3.1.5 because this is Salesforce's recommentadion, otherwise there might be connectivity issues. Also we have fixed an issue related to "Resume from last replay Id" feature of the Replay Topic and Replay Channel sources where if there were no events for 24 hours and a connection issue appeared, the reconnection was not working.
* Salesforce connector memory leak(CONN-1625) - there was an issue in Cached connection providers where some objects could not be cleared properly and this was leading to a memory leak 
