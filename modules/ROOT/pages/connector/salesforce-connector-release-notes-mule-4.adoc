...
*September 2018* -- v9.4.0 +
*October 2018* -- v9.3.4 with recent patch

_Select_

The Mule 4 Salesforce connector has two active versions.
...
* Fixed an issue for multiple operations(Batch info list, Batch info and Job info) that were going into an infinite loop because they were defaulting to XML content type. Some optional parameters were added so the user can specify the content type for the entity so that it will not fail anymore.
