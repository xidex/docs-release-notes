...
* *Outbound endpoint timeout doesn't work* - When calling an RFC that takes a long time to complete, the responseTimeout parameter is not considered and waits for the longest time. Not error or messages is triggered.
 Fixed by throwing an exception when timeout value is reached.

* *Outbound endpoint duplicate connection to SAP backend* - In case of error or timeout a second connection is triggered behind the scenes.
Fixed by removing of duplicate retry call in the exception handling block.
