...
|Issue |Description
// Fixed Issues 
| MULE-15756 | The HTTP Requester removes the payload when retrying. 
| MULE-15428 | Avoid unnecessary creation of `CaseInsensitive` maps during the HTTP request.
| MULE-15419 | HTTP: Reduce request creation overhead.
//  
// ------------------------------- 
...
* Revert: MULE-15128 - Proxy Config in HTTP Requester should not accept
