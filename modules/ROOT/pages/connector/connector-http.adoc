...
* Remove vulnerable commons-codec library. (https://www.mulesoft.org/jira/browse/MULE-16799[MULE-16799])
* Basic auth filter accepts header characters beyond the padding. (https://www.mulesoft.org/jira/browse/MULE-16898[MULE-16898])
* No transformers for `Iterator to byte[]` or `String` causes error. (https://www.mulesoft.org/jira/browse/MULE-15991[MULE-15991])
* Reduce logging verbosity when correlations IDs repeat but match. When both the `X-Correlation-ID` and `Mule-Correlation-ID` headers are present, Mule decides on one and logs a warning message about the decision. However, if in Mule 4 and 3 both headers are duplicated with the same value, Mule logs a debug message instead, and reduces the verbosity of apps. (https://www.mulesoft.org/jira/browse/MULE-17115[MULE-17115])
...
* Allow the encoding of URI parameters in the requester using the system property `mule.http.client.encodeUriParams`. (https://www.mulesoft.org/jira/browse/MULE-16772[MULE-16772])
* User credentials are shown in attributes `toString()`. (https://www.mulesoft.org/jira/browse/MULE-10525[MULE-10525])
* Mule throws error illegal group reference exception when trying to pass `$` in a URI Parameter in an HTTP request. (https://www.mulesoft.org/jira/browse/MULE-16669[MULE-16669])
...
* HTTP Connector does not implement equals and hash methods in the RequestHeader POJO which was causing memory leaks. (https://www.mulesoft.org/jira/browse/MULE-16652[MULE-16652])
