...
* Introduce `rawRequestUri` and `rawRequestPath` attributes when a request is received by an HTTP listener (encoded as received unlike `requestUri` and `requestPath` which are decoded). (https://www.mulesoft.org/jira/browse/MULE-16246[MULE-16246])
*  Remove overhead in event/message creation handling in policy scenarios. (https://www.mulesoft.org/jira/browse/MULE-16320[MULE-16320])
* Introduce logger for resolved dynamic HTTP requests: `org.mule.extension.http.internal.request.HttpRequestOperations` in `DEBUG` mode will log the final URI being used. (https://www.mulesoft.org/jira/browse/MULE-16295[MULE-16295])
* Add icon for the HTTP connector. (https://www.mulesoft.org/jira/browse/MULE-16378[MULE-16378])
  
=== Fixed Issues 
 
* Remove `sun.misc` package usages which are not available with Java 11 (https://www.mulesoft.org/jira/browse/MULE-15917[MULE-15917])
