...
* Salesforce Marketing Cloud Connector cannot retrieve DataExtensionObject(SE-10134) - This issue has been fixed by adding support for the ClientId property in the connector's config and also adding support for SQL like queries in the Retrieve operation. Now an user is able to select Native Query as the query to be used with this operation and he will be able to write queries on custom data extensions such as: SELECT PRODUCT_TYPE_CODE, MSG_CONTENT, CREATION_DATE  from DataExtensionObject[My_Custom_Data_Extension] WHERE CREATION_DATE > '2019-04-24T06:11:00.000+03:00'
