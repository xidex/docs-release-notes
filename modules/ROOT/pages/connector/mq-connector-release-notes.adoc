...
* Fixed an incorrect external API warning message about a connection being leaked.
...
* Fixed bug that prevented MUnit from providing the mock service for the Anypoint MQ connector inbound endpoint.
* Fixed warning log message stating that a connection was leaked.
* Fixed the threading configuration that produced invalid XML when using the wrong namespace.
* Fixed duplicated messages that occurred when a prefetched subscriber used the default TTL config.
* Fixed XML order generation that occurred when using the Anypoint Studio plugin.
