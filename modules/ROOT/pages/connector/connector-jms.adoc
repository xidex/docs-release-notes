...
* Fixed connection leaks. These leaks could happened when a Publish action failed internally
...
* Fixed deadlock when using the JMS Listener on too quick flows, which caused listeners to
stop receiving messages.

=== Fixed Issues

* Application hangs after receiving few messages from ActiveMQ in Mule 4.x. (https://www.mulesoft.org/jira/browse/MULE-16087[MULE-16087])
...
* JMS Connector doesn't initialize the TLS Context if is defined as a child element (https://www.mulesoft.org/jira/browse/MULE-15930[MULE-15930])
