...
* Salesforce Analytics upload parts concurrently has issues when INVALID_SESSION_ID is received (SE-9555) - Using the connector with the Batch Module was not working well when a connection issue appeared as the connector was not able to handle reconnection. This is now fixed.
