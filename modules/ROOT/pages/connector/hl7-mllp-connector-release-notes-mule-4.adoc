...
* Source configuration allows specifying a connection count, limiting the number of simultaneous connections to the supplied value. If not configured, the default behavior is to allow any number of simultaneous connections.

=== Fixed in this Release

* Closing a connection to the source would cause an empty message to be sent through the flow, generally resulting in an exception.
