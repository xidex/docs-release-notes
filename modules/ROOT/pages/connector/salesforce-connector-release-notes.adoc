...
* Added new connection providers in order to match the behavior of the Salesforce API(only 1 connection with the same credentials).
Added keep-alive functionality to these connection providers, meaning that the connector can try to keep the session alive even if it is not used(This functionality can be enabled/disabled by the users).
