...
These are the new features and enhancements, known limitations, issues, and fixes for Anypoint Visualizer. See xref:visualizer::index.adoc[Anypoint Visualizer] documentation for more information.

== June 7, 2019

=== New Features and Enhancements

Visualizer now detects running Mule applications that don't appear on the canvas and: 

* Lists the missing applications in each environment
* Outlines the reasons why Visualizer cannot support them in their current states
* Directs you to where you can update the applications to make them appear in the Visualizer canvas

If any apps are missing from a given environment, an icon and the number of excluded apps appear by the environment name in the side panel. Click the icon for more details.
