...
* In 3.7 message transform return an OuputHandler object. This approach (though it was good for streaming) introduces several issues specially with exeception handling and connectors support. Starting from 3.8 the default output is a InputStream, this stream is either in memory or disk based on the size. For most the cases this change will be backguards compatible, but for some edge cases where old behaviour is required it can be set by adding `mode="deferred"` at the message-transform xml element.
