...
This tool enables you to migrate server configuration data in your Mule Management Console (MMC) instance to a Runtime Manager instance. MMC is in the process of being fully replaced by the Runtime Manager. https://www.mulesoft.com/legal/versioning-back-support-policy#mule-management-console[MMC has been deprecated], so we advise that you carry out this migration at your soonest convenience. The migration process consists of executing two scripts and taking care of a couple of manual steps in between.

Download the migration tool (*mmc-arm-migrationscripts*) from https://support.mulesoft.com[the Support portal]. If you do not have credentials to log in, please contact your Customer Success Manager.
...
. Download the script file from https://support.mulesoft.com[the Support portal].
