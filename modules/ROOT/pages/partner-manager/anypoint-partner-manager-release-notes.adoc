...
* The Partner Manager APIs have been updated support passing an HTTP header (*x-entity-encoding*) to protect against potential XSS attacks when Partner Manager is used with custom-built HTML pages. 
Use of this header is optional; it is only needed if you are building a custom UI for the Partner Manager APIs and want the returned data to be html encoded.

* Fixed error in which Partner Manager Connector returns a “JSON property value too long” error when posting large error messages to the Tracker API.  The fix is to extend the maximum length of the field and truncate the message when it exceeds the maximum length.

=== New Features

* Added support for environment name resolution in audit logs.
...
* Added reporting of partner import, promotion, and export errors to users.
* Optimized performance of tracking data queries that were resulting in “service unavailable” messages in some cases.
