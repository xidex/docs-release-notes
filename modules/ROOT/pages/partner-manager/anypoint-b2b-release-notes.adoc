= Anypoint B2B Product Release Notes
ifndef::env-site,env-github[]
include::_attributes.adoc[]
endif::[]
:keywords: b2b, release notes

Access the entire release notes set for each Anypoint B2B product.

* xref:partner-manager/anypoint-partner-manager-release-notes.adoc[Partner Manager]
* xref:connector/partner-manager-connector-release-notes.adoc[Partner Manager Connector]
* xref:connector/as2-connector-release-notes.adoc[AS2 Connector]
* xref:connector/ftps-connector-release-notes.adoc[FTPS Connector]
* xref:connector/x12-edifact-modules-release-notes.adoc[X12/EDIFACT EDI Modules]
