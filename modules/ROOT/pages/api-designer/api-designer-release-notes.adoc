...
* <<API Designer 0.4.14 Release Notes>>
...
// SE-9588
* The mocking service returned a 404 error instead of the correct response if a third-level resource used a hyphen in its name. For example, this call `digital-users` is the third-level resource would cause the issue:
+
----
/channels/{channel-code}/digital-users/{digital-user-id}
----
// SE-8999
* The mocking service returned a 400 error for a PATCH request that met these two conditions:
** The request had a JSON body of a type that was defined in a library.
** The library defined the type with an `!include` tag.
