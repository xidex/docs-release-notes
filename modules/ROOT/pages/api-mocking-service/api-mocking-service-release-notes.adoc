...
// SE-9588
* The mocking service returned a 404 error instead of the correct response if a third-level resource used a hyphen in its name. For example, a call to this endpoint, in which `digital-users` is a third-level resource, would cause the issue:
+
----
...
// SE-8790
* In some cases, validation of required content was not working for request bodies. Instead, payloads were accepted from the API Console without any validation.
// SE-8745
* In some cases, the Mocking Service accepted requests that did not contain required parameters.
// SE-8666
* When OAuth 2.0 was being used for authentication, the Mocking Service required users to provide both an authorization header and an access token. However, these two forms of authorization are mutually exclusive.
...
* The mocking service did not return messages for the 400 response in with the correct styling.
...
* TLS 1.0 and 1.1 were still supported.
