...
+
The chosen data types must have the same built-in type. For example, if you choose two data types to create a union data type from, both data types must be strings, or both must be numbers, and so on. You must not create a union data type from a custom data type that is a string and another that is an object. If you do, you cannot edit other properties of the element that you have created the union datatype for.
...
// SE-10576
* API specifications that included binary files were not displayed properly in Exchange after being published.
// SE-11683
