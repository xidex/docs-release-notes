...
== Known Issue
// DOCS-4552
Because of a bug in a third-party library that is used for the validation of the XML in XSD schemas, the API editor can be inconsistent in displaying the error message `Could not validate XSD schema`. For example, you might or might not see the error message whenever you open a particular XSD schema.
