...
This release introduces two new features and fixes several issues. There is also one known issue.
...
// SE-11358
* When a datatype was declared as a union in the last `type` entry of a `types` declaration, the API console did not display the datatype. For example, the datatype `c` below would not appear in the API console because it is the last `type` declared in the list of `types`.
+
----
#%RAML 1.0 Library
usage:
...
----
+
However, if you moved the declaration of `c` (as in the example below), then `c` would appear in the API console.
+
----
#%RAML 1.0 Library
usage:
...
----
// DESIGNER-3619
* Importing an API specification from Exchange would overwrite newer versions existing assets in a project.
+
...
If you are not using Google Chrome as your web browser when using the API editor, clicking an information icon that appears next to an error message does not open a window to display information about the error.
