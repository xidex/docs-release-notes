...
+
Note: If you have two or more identical version names in one API group for example, two versions named "v1" in an API group, you can distinguish them post-upgrade by the Exchange asset ID. The Exchange asset versions shares the major number (1.x.x) but differ in the patch number (1.0.0 and 1.0.1). The latest is the one that has the "Select RAML" checkbox checked in the grouping step, otherwise a version's creation date determines the numbering.
+
