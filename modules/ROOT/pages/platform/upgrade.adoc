...
* User permissions model is now action-based, and set for users at the environment level, which is aligned to the rest of the management center. Administrators should set environment-level permissions for all users after the upgrade. 
...
Auto-discovery changes only apply to the newly created APIs and new API instances.

* Post-upgrade, you have to configure the Autodiscovery element only for new APIs or API instances. Retrieve the necessary values from the API and the UI:
