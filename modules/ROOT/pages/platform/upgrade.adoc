...
Yes, API policies remain in effect during the upgrade. You should not have to change these settings whether on-prem or cloud. After the upgrade it's not required to change those properties, existing ones will continue to work. Regardless of the deployment target, you need to provide clientID and clientSecret.
