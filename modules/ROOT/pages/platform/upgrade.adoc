...
.. Note: If you have two or more identical version names in one API group e.g. two versions named "v1" in an API group, you will be able to distinguish them post-upgrade by the Exchange asset ID. The Exchange asset versions will share the major number (1.x.x) but differ in the patch number (1.0.0 and 1.0.1). The latest will be the one that has the "Select RAML" checkbox checked in grouping step, otherwise the version creation date will determine the numbering.
+
