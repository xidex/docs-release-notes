...
* If you have two identical version names in an API group e.g. two versions named "v1" in an API group, you will be able to distinguish them post-upgrade by the Exchange asset ID. The Exchange asset versions will share the major number (1.x.x) but differ in the patch number (1.0.0 and 1.0.1). The highest will be the one that has the checkbox of master version checked.
